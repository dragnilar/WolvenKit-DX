...
namespace WolvenKit
{
    ...
    public partial class ChunkPropertyViewer : XtraUserControl
    {
        ...
        private bool ExpandOneLevel(int depth, List<VariableListNode> children, int currentLevel = 0)
        {
            ...
            foreach (var c in children)
                if (currentLevel == depth)
                {
                    ...
                    if ((NativeMethods.GetWindowLong(treeView.Handle, NativeMethods.GWL_STYLE) &
                         NativeMethods.WS_VSCROLL) == NativeMethods.WS_VSCROLL)
                    {
                        treeView.Collapse(c);
                        return false;
                    }
        ...
        {
            var sNodes = treeView.SelectedObjects.Cast<VariableListNode>().Where(item => item?.Variable != null)
                .ToList();
        ...
        {
            var tocopynodes = (from VariableListNode item in treeView.SelectedObjects
