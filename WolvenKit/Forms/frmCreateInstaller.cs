...
namespace WolvenKit.Forms
{
    public partial class frmCreateInstaller : Form
    {
        ...
        private void visualButton1_Click(object sender, EventArgs e)
        {
            ...
            {
                if (sf.ShowDialog() == DialogResult.OK) CreatePackage(sf.FileName);
            }
        }
        ...
        private void visualTextBox1_ButtonClicked()
        {
            ...
            var imageExtensions =
                string.Join(";", ImageCodecInfo.GetImageDecoders().Select(ici => ici.FilenameExtension));
        ...
        private async Task CreatePackage(string outpath)
        {
            ...
            var asm = WKPackage.CreateModAssembly(modversionTB.Text, modnameTB.Text,
                new Tuple<string, string, string, string, string, string>(authorTB.Text, donateTF.Text, webTF.Text,
                    facebookTF.Text, twitterTF.Text, youtubeTF.Text), descriptionRTB.Text, largedescRTB.Text,
                licenseTB.Text, new Tuple<Color, bool, Color>(headercolor, useblackCB.Checked, iconbg),
                new List<XElement>());
            var pkg = new WKPackage(asm, iconpathTB.Text,
